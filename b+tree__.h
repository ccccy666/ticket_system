...
struct point_ {//叶子上元素
    ...
    point_(const key &a, const T &b) : index(a), valu(b) {
//        index = a;
//        valu = b;
    }

    friend bool operator==(const point_ &a, const point_ &b) {
...
class b_plus_tree_ {
public:
    const int maxsize = std::max(10,2*(2048/(sizeof(key)+sizeof(T)))), minsize = maxsize>>1,m=maxsize+1;
    ...
    b_plus_tree_() = default;

    b_plus_tree_(std::string name) : file(name) {
        inout.open(file);
        if (!inout) {
            block_<key, T> tmp;
            std::ofstream out;
    ...
    void myre(int coord, block_<key, T> &node) {
        if (coord < 0) {
            exit(1);
        }
        inout.seekg(coord * sizeof(block_<key, T>) + 2*sizeof(int));
    ...
    void myread(int coord, block_<key, T> &node) {
        if (coord < 0) {
            exit(1);
        }
        inout.seekg(coord * sizeof(block_<key, T>) + 2*sizeof(int));
    ...
    void mywrite(int coord, block_<key, T> &node) {
        if (coord < 0) {
            exit(1);
        }
        //cout<<node.size;
        inout.seekp(coord * sizeof(block_<key, T>) + 2*sizeof(int));
        inout.write(reinterpret_cast<char *>(&node), sizeof(node));

    }

    void insert(point_<key, T> &ele) {
        block_<key, T> new_;
        //myread(0, new_);
...
//            }
    ...
    void split(int coord,block_<key,T> &node){
        total++;
        //num_of_block_++;
        block_<key,T> nod,tmp,tm;
        //block_<key,T> tmp;
    ...
    void balance_insert(block_<key,T> &first,block_<key,T>&second){
        block_<key,T>tmp;
        while(vec.size()){
...
//            cout<<second.ele[0].valu<<' '<<second.ele[1].valu;
            if(tmp.size>maxsize){
                ...
                block_<key,T>tp=split_find(tmp.place,tmp);
    ...
    block_<key,T> split_find(int coord,block_<key,T> &node){
        total++;
        block_<key,T> nod,tmp,tm;
...
//            printf("%s\n","null");
        ...
        for(int i=0;i<fin.size;i++){
            ...
            if((!fla)&&i==fin.size-1&&fin.next!=-1){
                myread(fin.next,fin);
                fla=1;
                i=-1;
            }
...
//        else printf("\n");
    ...
    void query_ticket(key begin,key end){

    }
    block_<key,T> getblock_(point_<key,T>k){
        //(ke,T());
        bool flag=0,fla=0;
        //compare cp;
        block_<key,T> fin;
...
//            }
    ...
    void adjust(block_<key,T>new_,point_<key,T>&ele){
        while(vec.size()){
            int pla=vec.back();
            vec.pop_back();
            //block_<key, T> new__;
    ...
    int bound(int low,int high,block_<key,T>&fin,point_<key,T>&ele){
        while(low<high){
            //if(fin.leaf)break;
            int mid=(low+high)>>1;
            if(ele<fin.ele[mid]){
...
//        cout<<fin.size<<' '<<fin.ele[0].valu<<' '<<fin.ele[1].valu<<'\n';
    ...
    void change(block_<key,T> node,point_<key,T>&past,point_<key,T>new_){
        while(node.parent!=-1){
    ...
    void dispose(block_<key,T> &node){
        block_<key,T>nxt;
        if(node.next!=-1){
            ...
            if(nxt.size>minsize){
                ...
                node.size++;
                node.maxi=nxt.ele[0];
                point_<key,T>ele=nxt.ele[1],el=nxt.ele[0];
                nxt.mini=nxt.ele[1];
                ...
                if(nxt.parent==node.parent){
                    ...
                    myread(node.pre,nxt);
                    if(nxt.size>minsize){
                        point_<key,T>ele=nxt.ele[nxt.size-1],el=node.ele[0];
    ...
    void merge(block_<key,T>&first,block_<key,T>&second){
        block_<key, T>tp;
    ...
    void balance_delete(block_<key,T>&first,block_<key,T>&second){
        block_<key,T>tmp,tm,son,pa;
        if(first.parent!=-1){
            ...
            if(tmp.place==root){
                ...
                if(tm.size>minsize&&tmp.parent==tm.parent){
                    ...
                    if(tm.parent==tmp.parent){
                        ...
                        myread(tmp.pre,tm);
                        if(tm.size>minsize){
                            point_<key,T> k=tm.ele[tm.size-1];
                            myread(tmp.parent,pa);
    ...
    void merge_find(block_<key,T> &first,block_<key,T> &second){
        block_<key,T>blk;
