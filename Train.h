...
public:
    list<seat>list_;
    b_plus_tree<train_seat,int>tr;
    Seats():tr("a_seats"),list_("seat_list"){}
    void insert(train_seat &ts,seat &se){
        list_.total++;
        point<train_seat,int>pp(ts,list_.total);
        tr.insert(pp);
        list_.mywrite(list_.total,se);
        //tr.insert(ele);
    }
    void find(train_seat &id,seat &se){
        int coord= tr.find(id);
        //seat se;
        list_.myread(coord,se);
    };
    block<train_seat,int> get_block(train_seat &id) {
...
public:
    ...
    void insert(train &acc) {
        list_.total++;
        point<trainID, int> p(acc.id, list_.total);
        tr.insert(p);
        list_.mywrite(list_.total,acc);
    }

    void find(trainID &id,train &tra) {

        int coord= tr.find(id);
        //train tra;
        list_.myread(coord,tra);
        //return tra;
    }
...
//
    void mywrite(block<trainID, int> &bk) {

        tr.mywrite(bk.place, bk);
    }

    block<trainID, int> getblock(trainID &id) {
        return tr.getblock(id);
    }
